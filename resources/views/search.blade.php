...
<body class="bg-zinc-50 pt-24 pb-12">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 sm:space-y-12 mb-16">
        ...
        <div>
            <a href="{{ route('home') }}" class="inline-flex items-center gap-2 text-sm font-bold text-zinc-500 hover:text-zinc-900 transition-colors mb-4">
            ...
            <p class="mt-2 text-zinc-500 font-medium text-sm">Menemukan {{ $scripts->total() }} script yang sesuai.</p>
        </div>

        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6">
            @forelse($scripts as $script)
                <a href="{{ route('script.show', $script->slug) }}" class="block bg-white rounded-xl sm:rounded-2xl border border-zinc-200 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group flex flex-col hover:-translate-y-1">
            ...
            @empty
                <div class="col-span-full text-center py-16 px-4 bg-white rounded-2xl border border-zinc-200 shadow-sm">
                    ...
                    <h3 class="text-lg font-bold text-zinc-900 mb-1">Pencarian Tidak Ditemukan</h3>
                    <p class="text-zinc-500 font-medium text-sm">Coba gunakan kata kunci lain atau periksa ejaanmu.</p>
                </div>
            @endforelse
        </div>
