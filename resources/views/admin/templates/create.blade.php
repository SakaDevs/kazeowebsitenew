<x-admin-layout>
    <x-slot name="title">Create Script Template</x-slot>
    ...
    <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden max-w-3xl">
        <div class="p-6 border-b border-zinc-200 bg-zinc-50/50">
            <h3 class="text-lg font-bold text-zinc-900">Add New Template</h3>
            <p class="text-sm text-zinc-500 font-medium mt-1">Buat template deskripsi baru untuk mempermudah upload script.</p>
        </div>
        ...
        <form action="{{ route('admin.templates.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" required autofocus placeholder="Contoh: Template Recall Animasi"
            ...
            <div class="space-y-1.5">
                ...
                <textarea id="content" name="content" rows="8" required placeholder="Tulis panduan atau deskripsi standarmu di sini..."
                    class="block w-full px-4 py-3.5 rounded-xl border border-zinc-200 bg-zinc-50/50 text-zinc-900 focus:bg-white focus:border-zinc-900 focus:ring-2 focus:ring-zinc-900/20 transition-all duration-300 resize-y">{{ old('content') }}</textarea>
